...
description: Exponer las funcionalidades de la app (no un LLM ni un agente) como tools de un servidor MCP en .NET, usando el SDK oficial ModelContextProtocol — para que clientes de IA (Claude.ai, Claude Desktop, Cursor) puedan invocar la app. Usar cuando se pide dar acceso a la app vía MCP; no aplica a nada relacionado con LLMs propios, agentes o embeddings.
---

# mcp-tool-server

Un servidor MCP en .NET que expone operaciones de la app (CRUD, consultas) como *tools* invocables por un
cliente de IA externo (Claude.ai, Claude Desktop, Cursor, etc.). Esto NO es "conectar la app a un LLM" — es lo
...
suficiente para uso personal/interno. Si el cliente MCP requiere un login interactivo (Claude.ai en modo
...
- Conectar un cliente MCP (Claude Desktop, o `npx @modelcontextprotocol/inspector http://localhost:<puerto>/mcp`)
