---
name: skill-author
description: Crear o editar otras skills de este proyecto en formato nativo de Claude Code (SKILL.md con frontmatter name+description). Usar cuando se descubre un patrón repetible propio de ESTE repo que conviene documentar como skill nueva, o cuando piden mejorar una existente.
---

# skill-author

Meta-skill para seguir construyendo esta suite. Una skill es conocimiento empaquetado para que Claude Code lo
...
`.claude/skills/<nombre-kebab-case>/SKILL.md` en la raíz del proyecto. Claude Code descubre automáticamente
...
description: <UNA línea, en tercera persona, que dice CUÁNDO usar la skill — Claude Code la lee para decidir si la carga. Incluí las palabras/situaciones gatillo dentro de la misma oración, no una lista aparte de triggers.>
---
...
- Solo `name` y `description` — Claude Code no necesita (ni usa) campos como `triggers` o `tools` separados;
...
4. Escribí el archivo en `.claude/skills/<nombre>/SKILL.md`.
...
- ❌ Reintroducir campos de frontmatter de otros formatos (`triggers`, `tools`) que Claude Code no usa.
